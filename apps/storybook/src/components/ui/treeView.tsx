...
const TreeView = React.forwardRef<HTMLDivElement, TreeProps>(
  ({ data, initialSelectedItemId, onSelectChange, expandAll, defaultLeafIcon, defaultNodeIcon, className, onDocumentDrag, ...props }, ref) => {
    ...
    return (
      <div className={cn('relative overflow-hidden rounded-lg border border-gray-200 bg-white p-3 dark:border-gray-700 dark:bg-gray-900', className)}>
        <TreeItem
          data={data}
          ref={ref}
  ...
  return (
    <AccordionPrimitive.Root type="multiple" value={value} onValueChange={s => setValue(s)}>
      <AccordionPrimitive.Item value={item.id}>
        ...
        <AccordionContent className="ml-5 border-l border-gray-200 pl-3 dark:border-gray-700">
          <TreeItem
...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Header>
    <AccordionPrimitive.Trigger
      ref={ref}
      className={cn('flex w-full flex-1 items-center py-2 transition-all first:[&[data-state=open]>svg]:first-of-type:rotate-90', className)}
      {...props}
    >
      <ChevronRight className="mr-1 h-4 w-4 shrink-0 text-gray-400 transition-transform duration-200 dark:text-gray-500" />
      {children}
    </AccordionPrimitive.Trigger>
  </AccordionPrimitive.Header>
));
...
const TreeActions = ({ children, isSelected }: { children: React.ReactNode; isSelected: boolean }) => {
  return <div className={cn(isSelected ? 'opacity-100' : 'opacity-0', 'ml-auto transition-opacity group-hover:opacity-100')}>{children}</div>;
};
